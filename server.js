...
async function getDailyQuotes() {
    ...
    // 오늘이 공휴일인지 체크 (설날, 추석 등 음력 명절은 필요 시 날짜를 수동 추가하거나, 요일 감성으로 대체됨)
    let specialEvent = holidays[`${month}-${date}`] || "";

    // AI에게 줄 프롬프트 (조건을 매우 구체적으로 수정)
    const systemPrompt = `
        당신은 몽환적이고 신비로운 '글숲'의 정원사입니다.
        방문객에게 하루의 피로를 풀어주고 영감을 주는 시적인 덕담 5개를 작성해주세요.
        
        [오늘의 정보]
        - 날짜: ${year}년 ${month}월 ${date}일
        - 요일: ${dayOfWeek}요일
        ${specialEvent ? `- 특별한 날: ${specialEvent}` : ''}
        
        [작성 규칙 - 매우 중요!]
        1. 오늘이 '${dayOfWeek}요일' 이라는 점(또는 '${specialEvent}')을 문장 속에 **분명하게, 하지만 감성적으로** 드러내세요. 
           (너무 추상적으로만 쓰지 말고, 요일이나 명절이 주는 현실적인 감각을 숲의 은유와 결합하세요.)
        2. "금요일", "월요일", "주말", "${specialEvent}" 같은 단어를 직접 사용해도 좋습니다. 단, 기계적인 챗봇 인사는 피하고 한 편의 짧은 시처럼 적어주세요.
        3. 5개의 문장이 각각 다른 느낌과 은유를 가지도록 다양하게 작성하세요.
        4. 문장 앞에 번호(1., 2. 등)나 기호(-, *)를 절대 붙이지 마세요. 각 문장은 줄바꿈(엔터)으로만 구분하세요.
        
        [좋은 예시 - 이 톤앤매너를 따르세요]
        - 월요일: "다시 시작되는 월요일의 아침, 정원의 맑은 이슬이 당신의 새로운 한 주를 조용히 응원합니다."
        - 수요일: "한 주의 절반을 쉼 없이 달려온 수요일, 오늘은 숲의 그늘에서 잠시 숨을 고르셔도 좋습니다."
        - 금요일: "치열했던 평일을 뒤로한 금요일 밤, 정원의 나무들도 당신과 함께 편안한 숨을 내쉽니다."
        - 주말(토/일): "여유가 흐르는 주말, 글숲의 바람이 당신의 어깨에 쌓인 먼지를 부드럽게 털어냅니다."
        - 기념일(예: 크리스마스): "크리스마스의 따뜻한 불빛이 글숲의 겨울 나무 위로 사뿐히 내려앉는 밤입니다."
    `;
    ...
    try {
        ...
        if (quotesArray.length > 0) {
            ...
            console.log(`[서버] ${todayStr} 덕담 5개 생성 완료!`);
        }
...
const analyzeLimiter = rateLimit({
    windowMs: 24 * 60 * 60 * 1000, // 24시간
    max: 20, // 넉넉하게 20번 (어차피 프론트에서 하루 2번으로 막음)
    message: { error: '오늘 일기 분석 요청 한도를 초과했습니다.' }
});
